...
class Puppeteer {
  ...
  dealTpl(name, data) {
    ...
    //console.log('data.resPath', data.resPath)
    /** 替换模板 */
    //console.log('data', this.html[tplFile], data)
    let tmpHtml = template.render(this.html[tplFile], data) // 这里出错？ 可能就是单纯的 yunzai的图 没做对  试试其他包吧 或者就不要yunzai，直接跑miaomiao试试，你试试，retime现在有了直接现在拉一个

    //console.log('savePath', tmpHtml)
    // 难道是写入错误？
    /** 保存模板 */
    try {
