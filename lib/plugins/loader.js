...
class PluginsLoader {
    ...
    for (let File of files) {
      ...
      try {
        ...
        if (error.stack.includes('Cannot find package')) {
          ...
          console.error(`载入插件错误：${File.name}`)
  ...
  packageTips(packageErr) {
    ...
    packageErr.forEach(v => {
      ...
      console.mark(`${v.File.name} 缺少依赖：${pack}`)
      console.mark(`请执行安装依赖命令：${'pnpm add ' + pack + ' -w'}`)
    })
  ...
  creatTask() {
    ...
    this.task.forEach(val => {
      val.job = schedule.scheduleJob(val.cron, async () => {
        try {
          if (val.log === true) {
            console.mark(`开始定时任务：${val.name}`)
          }
          let res = val.fnc()
          if (util.types.isPromise(res)) res = await res
          if (val.log === true) {
            console.mark(`定时任务完成：${val.name}`)
          }
        } catch (error) {
  ...
  watch(dirName, appName) {
    ...
    watcher.on('change', async path => {
      console.mark(`[修改插件][${dirName}][${appName}]`)
      ...
      try {
        ...
        console.error(`载入插件错误：${console.red(dirName + '/' + appName)}`)
        console.error(decodeURI(error.stack))
        return
      }
    ...
    watcher.on('unlink', async path => {
      console.mark(`[卸载插件][${dirName}][${appName}]`)
      for (let i in this.priority) {
        if (this.priority[i].key == key) {
  ...
  watchDir(dirName) {
    ...
    setTimeout(() => {
      ...
      watcher.on('add', async PluPath => {
        ...
        console.mark(`[新增插件][${dirName}][${appName}]`)
        let tmp = {}
        try {
          tmp = await import(
            `../../plugins/${dirName}/${appName}?${moment().format('X')}`
