...
class PluginsLoader {
  ...
  async load (isRefresh = false) {
    ...
    for (let File of files) {
      try {
        ...
        lodash.forEach(apps, (p, i) => {
          ...
          let plugin = new p()
          console.debug(`载入插件 [${File.name}][${plugin.name}]`)
          /** 执行初始化 */
          this.runInit(plugin)
          /** 初始化定时任务 */
        ...
        if (error.stack.includes('Cannot find package')) {
          ...
          console.error(`载入插件错误：${logger.red(File.name)}`)
          console.error(decodeURI(error.stack))
        }
      }
    }
    ...
    console.info(`加载定时任务[${this.task.length}个]`)
    console.info(`加载插件完成[${pluCount}个]`)
    console.info('-----------')
  ...
  packageTips (packageErr) {
    ...
    packageErr.forEach(v => {
      ...
      console.info(`${v.File.name} 缺少依赖：${logger.red(pack)}`)
      console.info(`新增插件后请执行安装命令：${logger.red('pnpm install -P')} 安装依赖`)
      console.info(`如安装后仍未解决可联系插件作者将 ${logger.red(pack)} 依赖添加至插件的package.json dependencies中，或手工安装依赖`)
    })
    // console.error('或者使用其他包管理工具安装依赖')
    console.info('---------------------')
  }
        ...
        if (plugin.rule) {
          for (let v of plugin.rule) {
            ...
            if (regExp.test(messageOrApplet)) {
              ...
              if (v.log !== false) {
                console.info(`${e.logFnc}${e.logText} ${lodash.truncate(e.msg, { length: 80 })}`)
              }
              ...
              try {
                ...
                if (res !== false) {
                  ...
                  this.setLimit(e)
                  if (v.log !== false) {
                    console.info(`${e.logFnc} ${lodash.truncate(e.msg, { length: 80 })} 处理完成 ${Date.now() - start}ms`)
                  }
                  break a
                }
              } catch (error) {
                console.error(`${e.logFnc}`)
  ...
  checkDisable (e, p) {
    ...
    if (!lodash.isEmpty(groupCfg.enable)) {
      ...
      // console.debug(`${e.logText}[${p.name}]功能已禁用`)
      return false
    }
  ...
  watch (dirName, appName) {
    ...
    watcher.on('change', async path => {
      console.info(`[修改插件][${dirName}][${appName}]`)

      let tmp = {}
      try {
        tmp = await import(`../../plugins/${dirName}/${appName}?${moment().format('x')}`)
      } catch (error) {
        console.error(`载入插件错误：${logger.red(dirName + '/' + appName)}`)
        console.error(decodeURI(error.stack))
        return
      }
    ...
    watcher.on('unlink', async path => {
      console.info(`[卸载插件][${dirName}][${appName}]`)
      for (let i in this.priority) {
        if (this.priority[i].key == key) {
  ...
  watchDir (dirName) {
    ...
    setTimeout(() => {
      ...
      watcher.on('add', async PluPath => {
        ...
        console.info(`[新增插件][${dirName}][${appName}]`)
        let tmp = {}
        try {
          tmp = await import(`../../plugins/${dirName}/${appName}?${moment().format('X')}`)
        } catch (error) {
        ...
        lodash.forEach(tmp, (p) => {
          if (!p.prototype) {
            console.error(`[载入失败][${dirName}][${appName}] 格式错误已跳过`)
            return
          }
          /* eslint-disable new-cap */
          let plugin = new p()
